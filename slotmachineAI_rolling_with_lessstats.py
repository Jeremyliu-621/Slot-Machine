...
def main():
    ...
    while wallet > 0 and number_of_spins < max_spins:
        ...
        print()
        print(f"Wallet: ${wallet:.2f} | Total Gain: ${total_gain:+.2f} | Total Loss: ${total_loss:.2f}")
        print(f"Number of spins: {number_of_spins} | Number of gains: {number_gains} | Number of losses: {number_losses}")
        print()
        print(f"Net revenue for gambler: ${(total_gain+total_loss):.2f} | Net revenue for casino: ${-(total_gain+total_loss):.2f}")
        try:
            print(f"AVG SPINS: (per gain: {(number_of_spins/number_gains):.2f}) | per loss: {(number_of_spins/number_losses):.2f}")
        except ZeroDivisionError:
            print(f"AVG SPINS: ZeroDivisionError")
